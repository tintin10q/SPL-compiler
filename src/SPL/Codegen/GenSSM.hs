...
import SPL.Colors (black, blue, bold, red, yellow)
...
outOfBoundExpectionRuntimeCode =
  [LABEL "'outOfBoundExpection"] <> printStringCode "\nArray out of bounds runtime expection:\nTrying to access .hd on an empty array. \nCaused by line "
    <> [TRAP 0]
    <> printStringCode " column "
    <> [TRAP 0, LDC (ord '.'), TRAP 1, HALT]
...
instance GenSSM (Program TypecheckedP) where
  generate program = do
    ...
    where
      isVarDecl f = case f of FunDecl {} -> False; VarDecl {} -> True
...
instance GenSSM (Decl TypecheckedP) where
  ...
  generate (VarDecl _ name _ expr) = do
    -- These are only global vars! Fun var decls are generated when we generate the the code for the fun decl
...
generatePrint ty = case {- Debug.trace (";; Printing the type " ++ show ty)-} ty of
  ...
  ListType BoolType ->
    let printBoolCode = generatePrint BoolType
        printBoolCodeSize = codeSize printBoolCode
     in [ LABEL "'printBoolList",
          LDC (ord '['),
          TRAP 1,
  ...
  ListType (ListType CharType) ->
    let printItemCode = generatePrint (ListType CharType)
        printItemCodeSize = codeSize printItemCode
     in [ LABEL "'printStringList",
          LDC (ord '['),
          TRAP 1,
          LDS 0, -- Check for empty list once
          LDA 0,
...
compareCode meta (TupleType t1a t1b) (TupleType t2a t2b) = do
  ...
  return $
    [ LDS 0,
      LDA (-1), -- Load the left of tuple 1
      LDS (-2),
      LDA (-1) -- Load the left of tuple 2
    ]
      <> aEqCode
      <> [BRT 4, LDC 0, BRA (8 + bEqCodeLen)] -- Leaves 1 single True or False
      -- If not equal we jump to the end and push false, else you have to compare the other item
      <> [ LDS 0,
           LDA 0, -- Load right of tuple 1
           LDS (-2),
           LDA 0 -- Load left of tuple 2
         ]
...
compareCode meta (ListType t1) (ListType t2) = do
  ...
  pure [LDS 0, LDA 0, LDC 0] <> generate Neq
    <> pure [LDS (-2), LDA 0, LDC 0] -- check if list 1 is at ts the end (but with Neq because then we can do and later)
    <> generate Neq
    <> pure [LDS 0, LDS (-2), XOR, BRF 6, AJS (-4), LDC 0, BRA (17 + cmpSize + 18)] -- check if list 2 is at the end
    <> pure -- If the xor is true (1,0 or 0,1) the whole result is false because not the same length, how do we clean up????
      [ AND {- We already know its the same because of the xor, so this either gets (1,1) or (0,0). So if the And is false we know there is no next value, If the and is true then the next value adresses are both NOT zero so there is a next value, we know there is a next value-},
        BRT 6,
        AJS (-2),
        LDC (-1),
        BRA (8 + cmpSize + 10 + 8)
      ]
    <> pure [LDS 0, LDA (-1), LDS (-2), LDA (-1)] -- Check if they are both 0 because if thats the case everything is true since we got here
    <> pure comparecode -- Load the values, using a copy of the adress so we can load the next adress next time
    <> pure [BRT 6, AJS (-2), LDC 0, BRA 10] -- compare the values,
    <> pure [LDA 0, AJS (-1), LDA 0, AJS 1] -- If its false jump to the end
    <> pure [BRA ((-10) - cmpSize - 52)] -- Move forward in both lists
    -- Otherwise jump back to the top which will actually consume the values?
...
compareCode meta t1@(TypeVar name True) t2@(TypeVar name' True) =
  if name == name'
    then
      warn
        ( "You are comparing two rigid type variables: " ++ show t1 ++ " and " ++ show t2
            ++ " at "
            ++ showStart meta
      ...
      warn
        ( "You are comparing a " ++ show t1 ++ " with a " ++ show t2 ++ " at " ++ showStart meta
...
compareCode meta t1@(TypeVar name False) t2@(TypeVar name' False) =
  if name == name'
    then
      warn
        ( "You are comparing two type variables: " ++ show t1 ++ " and " ++ show t2
            ++ " at "
            ++ showStart meta
...
compareCode meta t1@(TypeVar _ False) t2@(TypeVar _ True) =
  warn
    ( "You are comparing a " ++ show t1 ++ " with a " ++ show t2 ++ " at " ++ showStart meta
        ...
        ++ "This means that during runtime, if you compare a rigid typevar with a non rigid typevar the result will "
        ++ bold "always"
        ++ " be False. "
...
compareCode meta t1 t2 =
  warn ("You are comparing a " ++ show t1 ++ " with a " ++ show t2 ++ " at " ++ showStart meta ++ ". Because these are different types the result will ALWAYS be False. Make sure this is what you want.")
    >> pure [LDC 0]
...
instance GenSSM (Expr TypecheckedP) where
  ...
  generate (FunctionCallExpr (ty, _) func args) = do
    argcode <- generate args -- Last part removes arguments from the stack again!
